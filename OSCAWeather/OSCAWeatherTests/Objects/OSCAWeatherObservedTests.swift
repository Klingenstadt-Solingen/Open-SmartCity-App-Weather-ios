//
//  OSCAWeatherObservedTests.swift
//  OSCAWeatherTests
//
...
///  | pygmentize -g
///  ```
class OSCAWeatherObservedTests: OSCAParseClassObjectTestSuite<OSCAWeatherObserved> {
  override open func makeSpecificObject() -> OSCAWeatherObserved? {
    nil
  ...
  /// The file name has to match with the test class name: `OSCAWeatherObservedTests.json`
  override func testJSONDataAvailable() throws -> Void {
    try super.testJSONDataAvailable()
  }// end override func testJSONDataAvailable
#if DEBUG
  /// Is it possible to deserialize `JSON` scheme example data to an array  `OSCAWeatherObserved` 's?
  override func testDecodingJSONData() throws -> Void {
    try super.testDecodingJSONData()
  }// end override func testDecodingJSONData
#endif
  /// is it possible to fetch up to 1000 `OSCAWeatherObserved` objects in an array from network?
...
}// end class OSCAWeatherObservedTests
#endif
